<?php
/**
 * Проверка пользователей в базе данных (исправленная версия)
 */
...
try {
    ...
    if ($count['count'] > 0) {
        // Показываем список пользователей (без паролей) - используем только существующие поля
        $users = Database::fetchAll("SELECT user_id, user_login, user_fio, user_status, user_access_level, created_at FROM users ORDER BY user_id");
        
        echo "<h3>Список пользователей:</h3>";
        echo "<table border='1' style='border-collapse: collapse; margin: 10px 0;'>";
        echo "<tr><th>ID</th><th>Логин</th><th>ФИО</th><th>Статус</th><th>Уровень доступа</th><th>Дата создания</th></tr>";
        ...
        echo "<h3>Проверка паролей:</h3>";
        $usersWithPasswords = Database::fetchAll("SELECT user_id, user_login, user_password FROM users WHERE user_status = 1");
        foreach ($usersWithPasswords as $user) {
            if (empty($user['user_password'])) {
                echo "⚠ Пользователь " . htmlspecialchars($user['user_login']) . " (ID: " . $user['user_id'] . ") не имеет пароля<br>";
            } else {
                echo "✓ Пользователь " . htmlspecialchars($user['user_login']) . " (ID: " . $user['user_id'] . ") имеет пароль<br>";
            }
        }
    ...
    // Создаем тестового пользователя если их нет или если нет активных пользователей с паролями
    $activeUsersWithPasswords = Database::fetchAll("SELECT COUNT(*) as count FROM users WHERE user_status = 1 AND user_password IS NOT NULL AND user_password != ''");
    if ($count['count'] == 0 || $activeUsersWithPasswords[0]['count'] == 0) {
        ...
        try {
            Database::execute(
                "INSERT INTO users (user_login, user_password, user_fio, user_status, user_access_level, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                [
                    $testUser['user_login'],
...
echo "<p>Если пользователей нет или у них нет паролей, используйте созданного тестового пользователя:</p>";
echo "<ul>";
...
echo "<h2>6. Возможные проблемы</h2>";
echo "<ul>";
echo "<li>Проверьте, что у пользователей есть пароли</li>";
echo "<li>Убедитесь, что пользователи активны (user_status = 1)</li>";
echo "<li>Проверьте правильность логина и пароля</li>";
echo "<li>Убедитесь, что пароли захешированы с помощью password_hash()</li>";
echo "</ul>";
?> 